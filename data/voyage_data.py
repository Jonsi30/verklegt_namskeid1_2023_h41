...
class Voyage_Data:
    ...
    def get_all_voyage(self):
        ...
        with open(self.file_name, newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ret_list.append(Voyage_Model(row["id"], row["flight_nr"], row["dep_from"],
                                              row["arr_at"], row["dep_time"], row["arr_time"],
                                                row["dep_time_back"], row["arr_time_back"], row["plane_insignia"],
                                                  row["captain"], row["copilot"], row["head_of_service"], row["flight_attendant"]))
    ...
    def create_voyage(self, voyage):
        ...
        with open(self.file_name, 'a', newline='', encoding="utf-8") as csvfile:
            fieldnames = ["id", "flight_nr", "dep_from", 
                          "arr_at", "dep_time", "arr_time", 
                          "dep_time_back", "arr_time_back", "plane_insignia", 
                          "captain", "copilot", "head_of_service", "flight_attendant"]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writerow({'id': voyage[0],'flight_nr': voyage[1], 'dep_from': voyage[2], 'arr_at': voyage[3], 'dep_time': voyage[4], 'arr_time': voyage[5], 'dep_time_back': voyage[6], 'arr_time_back': voyage[7], 'plane_insignia': voyage[8], 'captain': voyage[9], 'copilot': voyage[10], 'head_of_service': voyage[11], 'flight_attendant': voyage[12] })
    ...
    def create_new_flights(self):
        ...
        with open(self.flight_list, 'a', newline='', encoding="utf-8") as csvfile:
            fieldnames = ["flight_nr", "dep_from", 
                          "arr_at", "departure", "arrival", 
                          "plane_insignia"]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            ...
            for voyage in all_voyages:
                ...
                writer.writerow({'flight_nr': voyage["flight_nr"], 'dep_from': voyage["dep_from"], 'arr_at': voyage["arr_at"], 'departure': voyage["dep_time"], 'arrival': voyage["arr_time"], 'plane_insignia': voyage["plane_insignia"]})
                
                writer.writerow({'flight_nr': flight_nr_back, 'dep_from': voyage["arr_at"], 
                                'arr_at': voyage["dep_from"], 'departure': voyage["dep_time_back"], 'arrival': voyage["arr_time_back"], 
