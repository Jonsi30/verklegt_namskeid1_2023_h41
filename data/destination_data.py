...
FIELDNAMES = ["Destination", "Distance from Iceland", "Flight Time", "Emergency Contact", "Emergency Contact Phone Number"]
...
class Destination_data:
    ...
    def get_all_destination(self):
        ...
        with open(self.file_name, newline='', encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ret_list.append(Destination(row["Destination"], row["Distance from Iceland"], row["Flight Time"], row["Emergency Contact"], row["Emergency Contact Phone Number"]))
        return ret_list
    ...
    def create_destination(self, destination: Destination):
        ...
        with open(self.file_name, 'a', newline='', encoding="utf-8") as csvfile:
            ...
            writer.writerow({'Destination': destination.destination,'Distance from Iceland': destination.distance_from_ice, 'Flight Time': destination.flight_time, 'Emergency Contact': destination.emergency_contact, 'Emergency Contact Phone Number': destination.emergency_contact_phone_nr })
    ...
    def update_destination(self, updated_dest: Destination):
        ...
        for dest in every_dest:
            if dest.destination == updated_dest.destination:
                new_list.append(Destination(updated_dest.destination, updated_dest.distance_from_ice, updated_dest.flight_time, updated_dest.emergency_contact, updated_dest.emergency_contact_phone_nr))
            else:
                new_list.append((dest.destination, dest.distance_from_ice, dest.flight_time, dest.emergency_contact, dest.emergency_contact_phone_nr))
